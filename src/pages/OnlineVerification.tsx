...
export default function OnlineVerification() {
  ...
  const { data: challenges = [], isLoading } = useQuery({
    queryKey: ['online-verification-challenges'],
    queryFn: async () => {
      const { data: journey, error: journeyError } = await supabase
        .from('journeys')
        .select('id')
        .eq('title', 'Online Information Verification')
        .maybeSingle();
      ...
      // Map the database challenges to the expected ChallengeType format
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <QuizHeader 
        title="Online Information Verification"
        currentQuestion={currentChallenge + 1}
        totalQuestions={challenges.length}
        scenario="Learn to verify online information and spot misinformation effectively"
      />
